...
describe('PermissionsController', () => {
  ...
  beforeEach(async () => {
    ...
    for (const entity of database.entityMetadatas) {
      const seq = entity.name.replace(/(?<!^)([A-Z])/g, (char) => '_' + char.toLowerCase()).toLowerCase() + '_id_seq';
  ...
  it('should get permissions', async () => {
    await database.manager.save(await Permission.create({
      account_id: '1',
      group_id: '1',
      action: 'read',
      recurso_id: '1'
    ...
    await database.manager.save(await Group.create({
      id: '1', 
      account_id: '1',
      name: 'grupo 1',
    }))

    await database.manager.save(await GroupUser.create({
      group_id: '1',
      user_id: '1',
    }, true))
